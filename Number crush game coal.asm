...
.data
		...
		WelcomeNoticeText3 byte  "|            @ Adeena @ Eesha             |",0
		WelcomeNoticeText4 byte  "|                                         |",0
...
mov eax, Score
    ...
    add esi, 4          ; Start storing digits from the end of the buffer
                        ; Assuming ScoreToString is 5 bytes: "    0" initially
                        ; esi will point to the null terminator if it's for 4 digits + null
                        ; A common way is to fill it from right to left.
                        ; For example, if ScoreToString is `byte 5 dup (" ")`,
                        ; you might `mov esi, offset ScoreToString + 4` to point to the last space.
                        ; Then `dec esi` before storing. Let's adjust for that common pattern.

    mov esi, offset ScoreToString + 3 ; Point to the last character cell for up to 4 digits
                                      ; if ScoreToString is `byte 5 dup (" ")`, this is index 3
    mov BYTE PTR [esi+1], 0           ; Ensure null termination at the end of the buffer

    mov ecx, 4          ; Max 4 digits if score < 10000 (adjust if needed)
Loop1:
    xor edx, edx        ; **Crucial: Clear EDX before 32-bit division EDX:EAX / EBX**
    div ebx             ; **EDX:EAX / EBX. Quotient in EAX, Remainder in EDX**
    push eax            ; Save quotient (EAX)
    add dl, 30h         ; Convert remainder (in DL now) to ASCII
    mov [esi], dl       ; Store ASCII digit
    dec esi             ; Move to previous char position in string
    pop eax             ; Restore quotient
    cmp eax, 0          ; Is quotient zero?
    je DoneConverting   ; If so, we are done
    cmp ecx, 1          ; Check if we've processed enough digits (to prevent infinite loop for 0)
                        ; or if you want to limit the number of digits.
    loop Loop1          ; Loop if quotient is not zero and ecx > 0

DoneConverting:
    ; If the loop finished because EAX became 0, but no digits were stored (e.g. Score was 0)
    ; you might need to store '0' explicitly.
    ; The current loop structure will store at least one digit if score is non-zero.
    ; If score was 0, it will jump to DoneConverting after the first div, and esi
    ; would not have been decremented after storing the '0'.

    ; If Score was 0, the first remainder (dl) would be 0, added 30h -> '0'.
    ; It gets stored at [esi]. Then eax is 0, jumps to DoneConverting.
    ; This seems okay.

    ; The original code had an issue here too. This is a more standard way.
    ; The `cmp eax, 10` and `jl Done` was for 8-bit div.
